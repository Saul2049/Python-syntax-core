...
from src.config.utils import merge_dict, flatten_config, unflatten_config
...
class TestDataTransformersRefactoring(unittest.TestCase):
    ...
    def test_fill_missing_values(self):
        """测试缺失值填充"""
        filled = MissingValueHandler.fill_missing_values(
            self.test_data_with_missing, method="mean"
        )
    ...
    def test_module_organization(self):
        ...
        total_expected = (
            len(NORMALIZERS) + 
            len(TIME_SERIES_TOOLS) + 
            len(SPLITTERS) + 
            len(MISSING_VALUE_TOOLS)
        )
    ...
    def test_advanced_functionality(self):
        ...
        # 测试滞后特征
        lagged = TimeSeriesProcessor.create_lagged_features(
            ts_data, ['price'], [1, 2, 3]
        )
        ...
        # 测试滚动特征
        rolling = TimeSeriesProcessor.create_rolling_features(
            ts_data, ['price'], [5, 10]
        )
