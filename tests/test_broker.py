...
def test_backtest_with_trailing_stop():
    # 创建简单的价格序列进行回测
    price = pd.Series([100.0, 101.0, 102.0, 103.0, 104.0, 105.0, 110.0, 
                       115.0, 120.0, 125.0, 130.0, 128.0, 126.0, 124.0])
    
    # 不使用移动止损的回测
    equity_no_trail = broker.backtest_single(price, fast_win=2, slow_win=4, atr_win=3,
                                      use_trailing_stop=False)
    
    # 使用移动止损的回测
    equity_with_trail = broker.backtest_single(price, fast_win=2, slow_win=4, atr_win=3,
                                        use_trailing_stop=True, breakeven_r=1.0, trail_r=2.0)
