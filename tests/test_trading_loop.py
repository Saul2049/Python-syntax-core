...
class TestTradingLoopModuleStructure:
    ...
    def test_module_all_list_completeness(self):
        """测试__all__列表的完整性"""
        # 获取模块中公开的函数和类
        public_items = [name for name in dir(src.trading_loop) 
...
class TestTradingLoopIntegration:
    ...
    @patch('src.trading_loop.trading_loop')
    @patch('builtins.print')
    @patch.dict('os.environ', {'TG_TOKEN': 'test_token', 'API_KEY': 'test_key', 'API_SECRET': 'test_secret'})
