...
class TestTradingLoopBackwardCompatibility(unittest.TestCase):
    ...
    def test_fetch_price_data_import(self):
        ...
        # 验证导入的函数具有相同的名称和可调用性
        self.assertEqual(trading_loop_module.fetch_price_data.__name__, original_func.__name__)
        self.assertTrue(callable(trading_loop_module.fetch_price_data))
    ...
    def test_calculate_atr_import(self):
        ...
        # 验证导入的函数具有相同的名称和可调用性
        self.assertEqual(trading_loop_module.calculate_atr.__name__, original_func.__name__)
        self.assertTrue(callable(trading_loop_module.calculate_atr))
    ...
    def test_get_trading_signals_import(self):
        ...
        # 验证导入的函数具有相同的名称和可调用性
        self.assertEqual(trading_loop_module.get_trading_signals.__name__, original_func.__name__)
        self.assertTrue(callable(trading_loop_module.get_trading_signals))
    ...
    def test_trading_loop_import(self):
        ...
        # 验证导入的函数具有相同的名称和可调用性
        self.assertEqual(trading_loop_module.trading_loop.__name__, original_func.__name__)
        self.assertTrue(callable(trading_loop_module.trading_loop))
    ...
    def test_trading_engine_import(self):
        ...
        # 验证导入的类具有相同的名称和是类类型
        self.assertEqual(trading_loop_module.TradingEngine.__name__, original_class.__name__)
        self.assertTrue(isinstance(trading_loop_module.TradingEngine, type))
...
class TestMainProgramBlock(unittest.TestCase):
    ...
    def test_main_with_all_env_vars(self, mock_print, mock_trading_loop):
        """测试有所有环境变量时的主程序执行"""
        # 设置所有必需的环境变量
        ...
        # 直接执行主程序逻辑
        import src.trading_loop as tl_module

        # 模拟 __name__ == "__main__" 条件
        if "TG_TOKEN" not in os.environ:
            mock_print("警告: 未设置TG_TOKEN环境变量，通知功能将不可用")

        if "API_KEY" not in os.environ or "API_SECRET" not in os.environ:
            mock_print("警告: 未设置API_KEY或API_SECRET环境变量，交易功能将不可用")

        # 启动交易循环
        tl_module.trading_loop()

        # 验证没有警告信息（因为所有环境变量都存在）
        warning_calls = [call for call in mock_print.call_args_list if "警告" in str(call)]
    ...
    @patch("src.trading_loop.trading_loop")
    @patch("builtins.print")
    def test_main_missing_tg_token(self, mock_print, mock_trading_loop):
        """测试缺少 TG_TOKEN 环境变量"""
        # 只设置 API 凭证
        os.environ["API_KEY"] = "test_key"
        os.environ["API_SECRET"] = "test_secret"
        ...
        # 验证有 TG_TOKEN 相关警告
        warning_calls = [call for call in mock_print.call_args_list if "TG_TOKEN" in str(call)]
        self.assertGreater(len(warning_calls), 0, "Should have TG_TOKEN warning")
    ...
    @patch("src.trading_loop.trading_loop")
    @patch("builtins.print")
    def test_main_missing_api_key(self, mock_print, mock_trading_loop):
        """测试缺少 API_KEY 环境变量"""
        # 只设置 TG_TOKEN 和 API_SECRET
        os.environ["TG_TOKEN"] = "test_token"
        os.environ["API_SECRET"] = "test_secret"
        # 确保 API_KEY 不存在
        ...
        api_calls = [
            ...
            if "API_KEY" in str(call) or "API_SECRET" in str(call)
        ]
        self.assertGreater(len(api_calls), 0, "Should have API_KEY warning")
    ...
    @patch("src.trading_loop.trading_loop")
    @patch("builtins.print")
    def test_main_missing_api_secret(self, mock_print, mock_trading_loop):
        ...
        self.assertGreater(len(api_calls), 0, "Should have API_SECRET warning")
    ...
    @patch("src.trading_loop.trading_loop")
    @patch("builtins.print")
    def test_main_missing_all_env_vars(self, mock_print, mock_trading_loop):
        """测试缺少所有环境变量"""
        # 清除所有相关环境变量
        ...
        # 验证有所有警告
        tg_calls = [call for call in mock_print.call_args_list if "TG_TOKEN" in str(call)]
        api_calls = [
            call
            for call in mock_print.call_args_list
            if "API_KEY" in str(call) or "API_SECRET" in str(call)
        ...
        self.assertGreater(len(tg_calls), 0, "Should have TG_TOKEN warning")
        self.assertGreater(len(api_calls), 0, "Should have API warning")
