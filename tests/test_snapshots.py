...
def test_backtest_snapshot_btc():
    ...
    # 允许0.01%的误差
    assert abs(final_equity - expected_equity) / expected_equity < 0.0001, \
...
def test_backtest_different_params(asset, fast_win, slow_win, atr_win, 
                                  use_trailing_stop, expected_equity):
    """表格测试：验证不同参数组合下的回测结果"""
    # 加载测试数据
    df = load_csv()
...
if __name__ == "__main__":
    ...
    btc_result = broker.backtest_single(
        df["btc"], fast_win=7, slow_win=20, atr_win=14, 
        risk_frac=0.02, init_equity=100_000.0, 
        use_trailing_stop=True, breakeven_r=1.0, trail_r=2.0, verbose=False
    )
    eth_result = broker.backtest_single(
        df["eth"], fast_win=7, slow_win=20, atr_win=14,
        risk_frac=0.02, init_equity=100_000.0,
        use_trailing_stop=True, breakeven_r=1.0, trail_r=2.0, verbose=False
    )
    ...
    btc_fast5 = broker.backtest_single(
        df["btc"], fast_win=5, slow_win=20, atr_win=14,
        risk_frac=0.02, init_equity=100_000.0,
        use_trailing_stop=True, breakeven_r=1.0, trail_r=2.0, verbose=False
    )
    ...
    btc_slow25 = broker.backtest_single(
        df["btc"], fast_win=7, slow_win=25, atr_win=14,
        risk_frac=0.02, init_equity=100_000.0,
        use_trailing_stop=True, breakeven_r=1.0, trail_r=2.0, verbose=False  
    )
    ...
    btc_no_trail = broker.backtest_single(
        df["btc"], fast_win=7, slow_win=20, atr_win=14,
        risk_frac=0.02, init_equity=100_000.0,
        use_trailing_stop=False, breakeven_r=1.0, trail_r=2.0, verbose=False
    )
