...
class TestTradingEngine:
    ...
    def setup_method(self):
        """测试设置"""
        with patch("src.monitoring.metrics_collector.get_metrics_collector") as mock_metrics:
            with patch("src.brokers.Broker") as mock_broker_class:
                with patch(
                    "src.core.signal_processor_vectorized.OptimizedSignalProcessor"
                ) as mock_processor:
                    mock_metrics.return_value = Mock()
                    mock_broker = Mock()
                    mock_broker.positions = {}  # 设置为字典而不是Mock
    ...
    def test_init_with_env_vars(self):
        ...
        with patch.dict(
            ...
            with patch("src.monitoring.metrics_collector.get_metrics_collector"):
                with patch("src.brokers.Broker"):
                    with patch("src.core.signal_processor_vectorized.OptimizedSignalProcessor"):
                ...
                with patch("time.time", side_effect=[1000.0, 1000.5]):
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            # 设置metrics上下文管理器
                            self.engine.metrics.measure_data_fetch_latency.return_value.__enter__ = (
                                Mock()
                            )
                            self.engine.metrics.measure_data_fetch_latency.return_value.__exit__ = (
                                Mock()
                            )
                            self.engine.metrics.measure_signal_latency.return_value.__enter__ = (
                                Mock()
                            )
                            self.engine.metrics.measure_signal_latency.return_value.__exit__ = (
                                Mock()
                            )
