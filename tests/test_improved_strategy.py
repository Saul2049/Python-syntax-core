...
class TestBuyAndHoldStrategy:
    ...
    def test_buy_and_hold_different_init_equity(self, sample_price_data):
        ...
        expected_final = init_equity + (sample_price_data.iloc[-1] - sample_price_data.iloc[0]) * position_size
...
class TestTrendFollowingStrategy:
    ...
    @patch('pandas.concat')
    def test_trend_following_basic(self, mock_concat, mock_trailing_stop, mock_stop_price, 
                                   mock_position_size, mock_ma, trending_price_data):
        """测试趋势跟踪策略基本功能"""
        # 设置模拟返回值
...
class TestImprovedStrategyIntegration:
    ...
    def test_multiple_strategies_same_data(self, mock_backtest, realistic_price_data):
        """测试多个策略在相同数据上的表现"""
        mock_backtest.return_value = pd.Series([120000] * len(realistic_price_data), 
                                               index=realistic_price_data.index)
