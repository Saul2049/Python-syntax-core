...
class W2Validator:
    ...
    def _validate_gc_configuration(self) -> bool:
        ...
        if config_check["thresholds_match"]:
            ...
            self.logger.error(
                f"❌ GC阈值错误: 当前{current_thresholds} vs 期望{expected_thresholds}"
            )
    ...
    def _analyze_gc_performance(self, stats: Dict) -> Dict:
        ...
        for gen, gen_stats in stats.get("by_generation", {}).items():
            gen_info = {
                ...
                "frequency_per_min": (
                    gen_stats.get("count", 0) / (stats.get("monitoring_duration", 1) / 60)
                ),
        ...
        perf_check.update(
            {
                ...
                "meets_w2_standards": (
                    p95_pause_ms <= self.w2_standards["max_p95_pause_ms"]
                    and gen0_freq <= self.w2_standards["max_gen0_rate_per_min"]
                    and gen2_freq <= self.w2_standards["max_gen2_rate_per_min"]
                ),
            }
        )
        ...
        self.logger.info(
            f"   Gen0频率: {gen0_freq:.2f}/min (标准: ≤{self.w2_standards['max_gen0_rate_per_min']}/min)"
        )
        self.logger.info(
            f"   Gen2频率: {gen2_freq:.2f}/min (标准: ≤{self.w2_standards['max_gen2_rate_per_min']}/min)"
        )
    ...
    def _generate_recommendations(self):
        ...
        if perf_check and not perf_check.get("meets_w2_standards", False):
            ...
            if (
                perf_check.get("gen0_frequency_per_min", 0)
                > self.w2_standards["max_gen0_rate_per_min"]
            ):
                recommendations.append(
                    {
                        "type": "performance",
            ...
            if (
                perf_check.get("gen2_frequency_per_min", 0)
                > self.w2_standards["max_gen2_rate_per_min"]
            ):
                recommendations.append(
                    {
                        "type": "performance",
    ...
    def _print_validation_summary(self):
        ...
        if perf_check:
            ...
            print(
                f"   Gen0频率: {perf_check.get('gen0_frequency_per_min', 0):.2f}/min (≤{self.w2_standards['max_gen0_rate_per_min']}/min)"
            )
            print(
                f"   Gen2频率: {perf_check.get('gen2_frequency_per_min', 0):.2f}/min (≤{self.w2_standards['max_gen2_rate_per_min']}/min)"
            )
...
async def main():
    ...
    parser.add_argument(
        "--duration", type=int, default=300, help="性能测试持续时间(秒) (默认: 300)"
    )
    parser.add_argument("--fix-config", action="store_true", help="自动修复GC配置问题")
