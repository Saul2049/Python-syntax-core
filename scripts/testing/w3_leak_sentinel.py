...
class W3LeakSentinel:
            ...
            while self.clean_hours_count < self.target_hours:
                ...
                if leak_detected:
                    ...
                    elapsed_hours = (datetime.now() - 
                                   (self.last_leak_detected or self.start_time)).total_seconds() / 3600
                ...
                self.logger.info(
                    f"⏰ 清洁小时数: {self.clean_hours_count}/{self.target_hours}"
                )
    ...
    def _analyze_memory_trend(self, checkpoints: List[LeakCheckpoint]) -> bool:
        ...
        if time_diff_minutes > 0:
            ...
            self.logger.debug(
                f"📊 内存增长率: {growth_rate:.3f} MB/min "
                f"(阈值: {self.memory_leak_threshold})"
            )
    ...
    def _analyze_fd_trend(self, checkpoints: List[LeakCheckpoint]) -> bool:
        ...
        if time_diff_minutes > 0:
            ...
            self.logger.debug(
                f"📊 FD增长率: {growth_rate:.3f} FD/min "
                f"(阈值: {self.fd_leak_threshold})"
            )
    ...
    def generate_w3_report(self) -> Dict:
        ...
        elapsed_hours = (current_time - self.start_time).total_seconds() / 3600 if self.start_time else 0
        
        # W3 验收检查
        w3_passed = (
            self.w3_status['completed'] and 
            self.clean_hours_count >= self.target_hours
        )
    ...
    def _analyze_overall_memory_trend(self) -> Dict:
        ...
        return {
            ...
            'average_growth_rate_mb_per_hour': round(total_memory_change / total_time_hours, 3) if total_time_hours > 0 else 0,
    ...
    def _print_w3_summary(self, report: Dict):
        ...
        print(f"🕛 清洁小时: {acceptance['achieved_clean_hours']}/{acceptance['target_clean_hours']}")
