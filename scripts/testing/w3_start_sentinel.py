...
class W3SentinelLauncher:
    ...
    async def start_sentinel(
        ...
        try:
            ...
            # 更新状态
            self._update_status(
                {"status": "running", "sentinel_started": datetime.now().isoformat()}
            )
            ...
            print("\n⏸️ 用户中断监控")
            self._update_status(
                {"status": "interrupted", "interrupt_time": datetime.now().isoformat()}
            )
            return False
            ...
            print(f"❌ W3 监控失败: {e}")
            self._update_status(
                {"status": "failed", "error": str(e), "error_time": datetime.now().isoformat()}
            )
            return False
...
def create_scheduled_config(run_name: str, cron_schedule: str = "0 3 * * *") -> str:
    ...
    config = {
        "w3_leak_sentinel_schedule": {
            ...
            "notifications": {"on_start": True, "on_completion": True, "on_failure": True},
            "environment": {
                "W3_TARGET_HOURS": "6",
...
async def main():
    ...
    parser.add_argument(
        "--run-name", type=str, default=None, help="运行名称标签 (默认: W3-timestamp)"
    )
    ...
    parser.add_argument("--status-only", action="store_true", help="仅显示状态，不启动监控")
    parser.add_argument("--create-schedule", type=str, help="创建定时任务配置 (cron表达式)")
