...
class CanaryDeployment:
    ...
    async def _collect_baseline_metrics(self) -> Dict[str, Any]:
        ...
        baseline = {
            ...
            'task_scheduling_p95': baseline_results.get('task_scheduling', {}).get('p95_task_latency_ms', 0),
            'cpu_usage_max': baseline_results.get('cpu_usage', {}).get('max_cpu_percent', 0),
            'throughput': baseline_results.get('async_signals', {}).get('throughput_signals_per_sec', 0)
        }
        
        self.logger.info(f"📈 基线指标: P95={baseline['async_signal_p95']:.1f}ms, 吞吐={baseline['throughput']:.1f}/s")
        return baseline
    ...
    async def _generate_deployment_report(self, baseline: Dict[str, Any], monitoring: Dict[str, Any]) -> Dict[str, Any]:
        """生成部署报告"""
    ...
    def _get_deployment_recommendation(self, success_rate: float, monitoring: Dict[str, Any]) -> str:
        """获取部署建议"""
