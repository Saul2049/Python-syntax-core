...
class W4StressTest:
    ...
    def simulate_trading_signal(self) -> float:
        ...
        for i in range(random.randint(30, 100)):  # 从50-200减少到30-100
            ...
            sma = sum([random.uniform(price * 0.95, price * 1.05) for _ in range(10)]) / 10  # 从20减少到10
    ...
    def update_status(self, status_update: Dict):
        ...
        status = {
            ...
            'progress_percent': (self.signals_processed / self.signals_target * 100) if self.signals_target > 0 else 0,
    ...
    def save_metrics(self):
        ...
        report = {
            ...
            'performance': {
                ...
                'p95_latency_ms': sorted(self.latencies)[-len(self.latencies)//20] if len(self.latencies) >= 20 else 0,
                'p99_latency_ms': sorted(self.latencies)[-len(self.latencies)//100] if len(self.latencies) >= 100 else 0,
    ...
    async def run_stress_test(self):
        ...
        self.logger.info(f"📡 信号频率: {target_frequency:.3f} 条/秒 (间隔: {signal_interval:.1f}秒)")
        ...
        try:
            while self.signals_processed < self.signals_target and datetime.now() < self.end_time:
                ...
                if self.signals_processed % 100 == 0:
                    ...
                    # 计算性能指标
                    recent_latencies = self.latencies[-100:] if len(self.latencies) >= 100 else self.latencies
                    avg_latency = sum(recent_latencies) / len(recent_latencies)
                    p95_latency = sorted(recent_latencies)[-len(recent_latencies)//20] if len(recent_latencies) >= 20 else avg_latency
                    ...
                    self.logger.info(f"📈 进度: {progress:.1f}% ({self.signals_processed}/{self.signals_target}), "
            ...
            self.logger.info(f"⏰ 用时: {(datetime.now() - self.start_time).total_seconds() / 3600:.1f} 小时")
            ...
            if not self.failed:
                ...
                validation = {
                    ...
                    'rss_under_limit': all(m.rss_mb <= self.max_rss_mb for m in self.metrics_history),
                    'latency_acceptable': all(m.p95_latency_ms <= 6.0 for m in self.metrics_history),
                    'no_critical_alerts': len([a for a in self.alerts if 'critical' in a.lower()]) == 0
                }
...
async def main():
    ...
    parser.add_argument('--run-name', type=str, default='W4-stress',
                       help='运行名称标签')
    parser.add_argument('--signals', type=int, default=20000,
                       help='目标信号处理数量')
    parser.add_argument('--duration', type=str, default='24h',
                       help='测试时长 (例: 24h, 1d)')
    parser.add_argument('--max-rss', type=int, default=40,
                       help='最大 RSS 限制 (MB)')
