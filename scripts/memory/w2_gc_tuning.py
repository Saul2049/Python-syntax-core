...
class W2GCTuner:
    ...
    async def _test_gc_configuration(
        ...
        p95_pause = max(
            gen0_stats.get('p95_pause', 0),
            gen2_stats.get('p95_pause', 0)
        ) * 1000  # 转换为毫秒
        ...
        if self.baseline_result:
            if self.baseline_result.p95_pause_ms > 0:
                improvement = ((self.baseline_result.p95_pause_ms - p95_pause) / 
                             self.baseline_result.p95_pause_ms) * 100
    ...
    async def run_progressive_tuning(self) -> Optional[GCTestResult]:
        ...
        for gen0_threshold in [900, 1200, 1500]:
            result = await self._test_gc_configuration(
                (gen0_threshold, 10, 10),
                f"gen0_{gen0_threshold}",
                f"Gen0={gen0_threshold}"
            )
    ...
    def generate_report(self) -> Dict:
        ...
        report = {
            ...
            'baseline_result': {
                ...
                'thresholds': self.baseline_result.thresholds if self.baseline_result else (700, 10, 10)
            },
            'best_result': {
