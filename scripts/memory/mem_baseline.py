...
class MemoryBaseline:
    ...
    async def _finalize_baseline(self):
        ...
        # 计算基线统计
        memory_samples = [s for s in self.baseline_data['memory_samples'] if 'error' not in s.get('memory_snapshot', {})]
...
def compare_with_baseline(current_data: Dict[str, Any], baseline_file: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'memory_rss' in current_stats and 'memory_rss' in baseline_stats:
            ...
            avg_change = ((current_rss['avg_mb'] - baseline_rss['avg_mb']) / baseline_rss['avg_mb']) * 100
            p95_change = ((current_rss['p95_mb'] - baseline_rss['p95_mb']) / baseline_rss['p95_mb']) * 100
        ...
        if 'cpu_usage' in current_stats and 'cpu_usage' in baseline_stats:
            ...
            cpu_change = ((current_cpu['avg_percent'] - baseline_cpu['avg_percent']) / baseline_cpu['avg_percent']) * 100
    ...
    try:
        ...
        if args.compare and os.path.exists(args.compare):
            ...
            if 'error' not in comparison:
                print(f"📈 内存变化: {comparison['memory_comparison'].get('avg_mb_change', 0):+.1f}%")
                print(f"🖥️ CPU变化: {comparison['cpu_comparison'].get('avg_percent_change', 0):+.1f}%")
