...
class W1CacheBenchmark:
    ...
    async def run_baseline_test(self) -> Dict[str, Any]:
        ...
        if self.fast_mode:
            ...
            baseline_task = asyncio.create_task(
                baseline.collect_baseline(self.test_duration)
            )
            
            strategy_task = asyncio.create_task(
                self._simulate_strategy_load(use_cache=False)
            )
            
            baseline_data, strategy_stats = await asyncio.gather(
                baseline_task, strategy_task
            )
    ...
    async def run_optimized_test(self) -> Dict[str, Any]:
        ...
        if self.fast_mode:
            ...
            strategy_task = asyncio.create_task(
                self._simulate_strategy_load(use_cache=True)
            )
    ...
    def generate_report(self, comparison: Dict) -> str:
        ...
        report.append(f"   变化量:  {mem_comp['rss_delta_mb']:+.1f} MB ({mem_comp['rss_change_percent']:+.1f}%)")
        ...
        report.append(f"   分配率降低 ≥ 20%:   {'✅' if acceptance['allocation_reduction_pass'] else '❌'}")
        report.append(f"   总体通过:           {'✅ PASS' if acceptance['overall_pass'] else '❌ FAIL'}")
    ...
    async def run_full_benchmark(self) -> bool:
        ...
        try:
            ...
            if not self.fast_mode:
                ...
                with open(f'output/w1_cache_benchmark_{mode_name.lower()}_{timestamp}.json', 'w') as f:
