...
class GCProfiler:
    ...
    def _init_prometheus_metrics(self):
        ...
        try:
            ...
            self.gc_pause_duration = Summary(
                f"gc_pause_seconds_{suffix}", "GC暂停时间", ["generation"]
            )

            self.gc_collections_total = Counter(
                f"gc_collections_total_{suffix}", "GC回收次数", ["generation"]
            )

            self.gc_collected_objects = Counter(
                f"gc_collected_objects_total_{suffix}", "GC回收对象数量", ["generation"]
            )
            ...
            self.gc_objects_tracked = Gauge(
                f"gc_objects_tracked_{suffix}", "当前追踪的对象数量", ["generation"]
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_gc_counts": {"gen0": gc_counts[0], "gen1": gc_counts[1], "gen2": gc_counts[2]},
            "gc_thresholds": {
        ...
        if gen0_stats.get("count", 0) > 0:
            ...
            if frequency > 10:  # 每秒>10次GC太频繁
                ...
                recommendations["rationale"][
                    "gen0"
                ] = f"Gen0频率过高({frequency:.1f}/s)，建议增加阈值"
                ...
                recommendations["rationale"][
                    "gen0"
                ] = f"Gen0暂停时间过长({avg_pause*1000:.1f}ms)，建议减少阈值"
    ...
    def save_report(self, filename: str):
        ...
        report = {
            ...
            "events": (
                self.gc_events[-100:] if len(self.gc_events) > 100 else self.gc_events
            ),  # 最近100个事件
        }
