...
class HealthChecker:
    ...
    def check_memory(self) -> HealthStatus:
        ...
        try:
            ...
            return HealthStatus("memory", "CRITICAL", f"Memory check failed: {e}", 
    ...
    def check_cpu(self) -> HealthStatus:
        ...
        try:
            ...
            return HealthStatus("cpu", "CRITICAL", f"CPU check failed: {e}", 
                              {"error": str(e)}, time.time())
    ...
    def check_disk(self) -> HealthStatus:
        ...
        try:
            ...
            return HealthStatus("disk", "CRITICAL", f"Disk check failed: {e}", 
                              {"error": str(e)}, time.time())
    ...
    def check_file_descriptors(self) -> HealthStatus:
        ...
        try:
            ...
            if status == "OK":
                ...
                message = f"File descriptors elevated: {num_fds} open (>{self.thresholds['fd_warning']})"
            else:
                message = f"File descriptors critical: {num_fds} open (>{self.thresholds['fd_critical']})"
            
            return HealthStatus("file_descriptors", status, message, details, time.time())
            
        except Exception as e:
            return HealthStatus("file_descriptors", "WARNING", f"FD check failed: {e}", 
                              {"error": str(e)}, time.time())
    ...
    def check_gc_health(self) -> HealthStatus:
        ...
        try:
            ...
            return HealthStatus("gc", "WARNING", f"GC check failed: {e}", 
                              {"error": str(e)}, time.time())
    ...
    def check_processes(self) -> HealthStatus:
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    if any('stress' in str(cmd).lower() or 'trading' in str(cmd).lower() 
                           for cmd in proc.info['cmdline'] or []):
                        trading_processes.append({
                            'pid': proc.info['pid'],
                            'name': proc.info['name'],
                            'cmdline': ' '.join(proc.info['cmdline'] or [])[:100]  # 截断长命令行
                        })
            ...
            return HealthStatus("processes", "WARNING", f"Process check failed: {e}", 
                              {"error": str(e)}, time.time())
