...
class MetricsCollector:
    ...
    def add_metric(self, name: str, value: Union[float, int], 
                   ...
                   help_text: str = "", metric_type: str = "gauge") -> 'MetricsCollector':
        """添加自定义指标（链式调用）"""
    ...
    def collect_system_metrics(self) -> 'MetricsCollector':
        ...
        try:
            ...
            self.add_metric("process_memory_rss_bytes", memory_info.rss, 
                          help_text="Resident Set Size in bytes", metric_type="gauge")
            self.add_metric("process_memory_vms_bytes", memory_info.vms, 
                          help_text="Virtual Memory Size in bytes", metric_type="gauge")
            
            # CPU指标
            cpu_percent = self.process.cpu_percent()
            self.add_metric("process_cpu_usage_percent", cpu_percent,
                          help_text="CPU usage percentage", metric_type="gauge")
            
            # 文件描述符
            try:
                num_fds = self.process.num_fds() if hasattr(self.process, 'num_fds') else 0
                self.add_metric("process_open_fds", num_fds,
            ...
            self.add_metric("system_memory_usage_percent", system_memory.percent,
                          help_text="System memory usage percentage", metric_type="gauge")
            
            # 运行时长
            uptime = time.time() - self.start_time
            self.add_metric("process_uptime_seconds", uptime,
                          help_text="Process uptime in seconds", metric_type="counter")
            
        except Exception as e:
            # 错误指标
            self.add_metric("metrics_collection_errors_total", 1,
                          labels={"error_type": type(e).__name__},
                          help_text="Total metrics collection errors", metric_type="counter")
    ...
    def collect_gc_metrics(self) -> 'MetricsCollector':
        ...
        try:
            ...
            for i, stats in enumerate(gc_stats):
                self.add_metric("gc_collections_total", stats.get('collections', 0),
                              labels={"generation": str(i)},
                              help_text="Total GC collections", metric_type="counter")
                self.add_metric("gc_collected_total", stats.get('collected', 0),
                              labels={"generation": str(i)},
                              help_text="Total GC collected objects", metric_type="counter")
                self.add_metric("gc_uncollectable_total", stats.get('uncollectable', 0),
                              labels={"generation": str(i)},
                              help_text="Total GC uncollectable objects", metric_type="counter")
            
            # GC阈值
            thresholds = gc.get_threshold()
            for i, threshold in enumerate(thresholds):
                self.add_metric("gc_threshold", threshold,
                              labels={"generation": str(i)},
                              help_text="GC threshold by generation", metric_type="gauge")
            
            # GC计数
            counts = gc.get_count()
            for i, count in enumerate(counts):
                self.add_metric("gc_objects_count", count,
                              labels={"generation": str(i)},
                              help_text="Objects count by GC generation", metric_type="gauge")
        
        except Exception as e:
            self.add_metric("gc_metrics_errors_total", 1,
                          labels={"error_type": type(e).__name__},
                          help_text="Total GC metrics errors", metric_type="counter")
        
        return self
    
    def collect_trading_metrics(self, additional_metrics: Optional[Dict[str, Any]] = None) -> 'MetricsCollector':
        """收集交易相关指标"""
...
def create_metrics_snapshot(process_name: str = "trading_system", 
                          trading_metrics: Optional[Dict[str, Any]] = None) -> str:
    """快速创建指标快照（一行调用）"""
