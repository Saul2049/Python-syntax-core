...
class MarketDataManager:
    ...
    def __init__(self, primary_provider: MarketDataProvider, backup_provider: MarketDataProvider, min_switch_interval: int = 300):
        self.primary = primary_provider
        self.backup = backup_provider
        self.using_primary = True
        self.last_switch_time = 0
        self.min_switch_interval = min_switch_interval  # 最小切换间隔(秒)
        logger.info(f"初始化市场数据管理器: 主源={primary_provider.get_name()}, 备源={backup_provider.get_name()}")
    ...
    def get_klines(self, symbol: str, interval: str, limit: int = 100) -> List[List]:
        ...
        if not result and self.using_primary:
            logger.warning(f"主数据源({self.primary.get_name()})返回空数据，尝试切换到备源({self.backup.get_name()})")
        ...
        if self.using_primary:
            if self.primary.is_available():
                ...
                if now - self.last_switch_time > self.min_switch_interval:
                    logger.warning(f"主数据源({self.primary.get_name()})不可用，切换到备源({self.backup.get_name()})")
                    self._switch_to_backup()
                    return self.backup
                else:
                    # 未达到切换间隔，继续使用主源
                    logger.warning(f"主数据源({self.primary.get_name()})不可用，但未达到切换间隔，继续尝试")
                    return self.primary
        else:
            # 当前使用备源，检查主源是否恢复
            if self.primary.is_available() and now - self.last_switch_time > self.min_switch_interval:
