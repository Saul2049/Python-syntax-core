...
class DataFetcher:
    ...
    def _get_binance_client(self):
        ...
        if self._binance_client is None:
            ...
            self._binance_client = BinanceClient(
                api_key=api_key, api_secret=api_secret, testnet=self.config.use_binance_testnet()
            )
        return self._binance_client

    def fetch_historical_data(
        self, symbol: str, interval: str = "1d", days: int = 365, source: str = "binance"
    ) -> pd.DataFrame:
        """
    ...
    def fetch_multiple_symbols(
        self, symbols: List[str], interval: str = "1d", days: int = 365, source: str = "binance"
    ...
    try:
        if args.multiple:
            ...
            for symbol, data in data_dict.items():
                if not data.empty:
                    output_path = (
                        args.output or f"{symbol}_{args.interval}_{args.days}d.{args.format}"
                    )
                    fetcher.save_data(data, output_path, args.format)
                    print(f"Saved {symbol} data to {output_path}")
        else:
            # Fetch single symbol
            data = fetcher.fetch_historical_data(args.symbol, args.interval, args.days, args.source)
