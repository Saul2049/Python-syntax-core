...
eq_df_btc_biased["Portfolio"] = (
    eq_df_btc_biased["btc"] + eq_df_btc_biased["eth"]
)
eq_total_btc_biased = eq_df_btc_biased["Portfolio"]

# 100% BTC
eq_btc_only = backtest_portfolio({"btc": df["btc"]}, init_equity=100_000)[
    "Portfolio"
]

# 100% ETH
eq_eth_only = backtest_portfolio({"eth": df["eth"]}, init_equity=100_000)[
    "Portfolio"
]
...
plt.plot(
    eq_total_equal, label="Equal Weight (50/50)", linewidth=2, linestyle="--"
)
...
plt.plot(
    btc_weights, label="BTC Weight (Standard)", linewidth=2, color="green"
)
...
plt.plot(
    btc_weights_bear,
    label="BTC Weight (Bear Defense)",
    linewidth=2,
    color="red",
)

plt.axhline(
    y=0.5, color="blue", linestyle="--", alpha=0.5, label="Equal Weight (0.5)"
)
plt.axhline(
    y=0.8, color="orange", linestyle="--", alpha=0.5, label="Fixed 80/20 (0.8)"
)
