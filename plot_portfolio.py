...
# 不同配置的投资组合回测
# 等权重组合 (50/50固定权重)
eq_df_equal = backtest_portfolio(
    prices_dict, 
...
# 100% BTC
eq_btc_only = backtest_portfolio(
    {"btc": df["btc"]},
    init_equity=100_000
)['Portfolio']

# 100% ETH
eq_eth_only = backtest_portfolio(
    {"eth": df["eth"]},
    init_equity=100_000
)['Portfolio']
...
print(f"Equal Weight:       CAGR: {metrics.cagr(eq_total_equal):.2%}, MaxDD: {metrics.max_drawdown(eq_total_equal):.2%}, Sharpe: {metrics.sharpe_ratio(eq_total_equal):.2f}")
print(f"Fixed 80/20:        CAGR: {metrics.cagr(eq_total_btc_biased):.2%}, MaxDD: {metrics.max_drawdown(eq_total_btc_biased):.2%}, Sharpe: {metrics.sharpe_ratio(eq_total_btc_biased):.2f}")
print(f"Dynamic Weight:     CAGR: {metrics.cagr(eq_total_dynamic):.2%}, MaxDD: {metrics.max_drawdown(eq_total_dynamic):.2%}, Sharpe: {metrics.sharpe_ratio(eq_total_dynamic):.2f}")
print(f"Bear Defense:       CAGR: {metrics.cagr(eq_total_dynamic_bear):.2%}, MaxDD: {metrics.max_drawdown(eq_total_dynamic_bear):.2%}, Sharpe: {metrics.sharpe_ratio(eq_total_dynamic_bear):.2f}")
