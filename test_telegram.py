...
def setup_parser():
    ...
    parser.add_argument('--message', type=str, default='✅ 交易系统通知测试成功!',
                      help='要发送的测试消息 (默认: ✅ 交易系统通知测试成功!)')
    parser.add_argument('--full-test', action='store_true', 
                      help='直接运行完整通知测试，无需交互确认')
    parser.add_argument('--debug', action='store_true',
                      help='显示详细的调试信息和API响应')
    parser.add_argument('--format', choices=['text', 'html', 'markdown'], default='html',
...
def send_telegram(token, chat_id, message, parse_mode='HTML', debug=False, retry=0):
    ...
    try:
        ...
        if debug:
            ...
            print(f"DEBUG - 请求参数: {json.dumps(payload, ensure_ascii=False, indent=2)}")
...
def main():
    ...
    if success and (args.full_test or (
        not args.full_test and input("是否要测试完整的交易通知系列? (Y/n): ").strip().lower() in ['', 'y', 'yes']
    )):
        print("\n测试交易通知序列...")
