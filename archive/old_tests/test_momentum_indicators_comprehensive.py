...
class TestMomentumIndicator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.price_data = pd.Series(
            [100.0, 101.5, 99.8, 102.3, 104.1, 103.7, 105.2, 106.8, 105.9, 107.5]
        )
...
class TestRateOfChange(unittest.TestCase):
    ...
    def test_rate_of_change_basic(self):
        ...
        expected_fourth = (
            (self.test_data.iloc[3] - self.test_data.iloc[0]) / self.test_data.iloc[0]
        ) * 100
    ...
    def test_rate_of_change_period_1(self):
        ...
        expected_second = (
            (self.test_data.iloc[1] - self.test_data.iloc[0]) / self.test_data.iloc[0]
        ) * 100
...
class TestMomentumIndicatorsIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 创建对应的高低价
        self.realistic_high = self.realistic_prices * (
            1 + abs(np.random.normal(0, 0.01, len(prices)))
        )
        self.realistic_low = self.realistic_prices * (
            1 - abs(np.random.normal(0, 0.01, len(prices)))
        )
