...
class TestMainBlockExecution:
    ...
    def test_main_block_full_execution(self, mock_signals, mock_broker, mock_metrics, 
                                     mock_read_csv, mock_plt):
        """测试完整的主程序块执行路径"""
        # Setup mock data
        ...
        mock_df = pd.DataFrame({
            'btc': btc_prices,
            'eth': 3000 + np.random.randn(500) * 300
        }, index=dates)
        mock_read_csv.return_value = mock_df
        
        # Mock broker methods
        mock_broker.backtest_single.return_value = pd.Series(
            100000 * (1 + np.random.randn(500) * 0.01).cumprod(), 
            index=dates
        )
...
class TestTrendFollowingAdvanced:
    ...
    def test_trend_following_stop_loss_execution(self, mock_broker):
        ...
        declining_prices = pd.Series([50000, 49000, 48000, 45000, 44000], 
                                   index=self.dates[:5])
...
class TestMainBlockMetricsAndVisualization:
    ...
    def test_main_block_visualization(self, mock_show, mock_savefig, mock_ylabel, 
                                    mock_title, mock_grid, mock_legend, mock_plot, mock_figure):
        """测试主程序块的可视化代码"""
    ...
    def test_final_performance_summary_logic(self, mock_metrics):
        ...
        best_equity = pd.Series(
            [100000] + list(100000 * (1 + np.random.randn(99) * 0.01).cumprod()),
            index=dates
        )
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_missing_column_handling(self):
        ...
        short_prices = pd.Series([50000, 51000], 
    ...
    def test_insufficient_data_handling(self):
        ...
        minimal_prices = pd.Series([50000], 
    ...
    def setup_method(self):
        """Setup test data"""
        self.prices = pd.Series([50000, 51000, 52000, 51500, 50500], 
...
class TestParameterValidation:
    ...
    def test_buy_and_hold_parameter_validation(self):
        """测试buy_and_hold的参数验证"""
        prices = pd.Series([50000, 51000, 52000], 
    ...
    def test_trend_following_parameter_validation(self, mock_broker):
        ...
        prices = pd.Series([50000, 51000, 52000, 53000, 54000], 
                         index=pd.date_range('2020-01-01', periods=5, freq='D'))
    ...
    def test_improved_ma_cross_parameter_validation(self, mock_broker):
        ...
        mock_broker.backtest_single.return_value = pd.Series([1000, 1100, 1200], 
                                                           index=prices.index)
...
class TestTargetedCoverageBoost:
    ...
    def setup_method(self):
        ...
        self.stop_loss_trigger_prices = pd.Series([
            50000, 51000, 52000, 53000, 54000,  # Rising to trigger entry
            53500, 53000, 52500, 52000, 47000,  # Falling to trigger stop loss  
            46000, 45000, 44000, 43000, 42000   # Continue falling
        ], index=self.dates[:15])
    ...
    def test_position_with_ma_exit_trigger(self, mock_broker):
        ...
        ma_exit_prices = pd.Series([
            50000, 52000, 54000, 56000, 58000,  # Rising trend
            57000, 55000, 53000, 51000, 49000,  # Falling below MA
            47000, 45000  # Continue falling
        ], index=self.dates[:12])
    ...
    def test_trailing_stop_update_branch(self, mock_broker):
        ...
        trailing_prices = pd.Series([
            50000, 52000, 54000, 56000, 58000, 60000, 61000, 62000
        ], index=self.dates[:8])
    ...
    def test_entry_condition_with_finite_atr(self, mock_broker):
        ...
        entry_prices = pd.Series(
            [50000] + [50000 + i * 100 for i in range(1, 50)],  # Rising trend
            index=self.dates[:50]
        )
    ...
    def test_equity_calculation_with_position(self, mock_broker):
        ...
        sustained_prices = pd.Series(
            [50000 + i * 50 for i in range(30)],  # Steady uptrend
            index=self.dates[:30]
        )
    ...
    def test_buy_and_hold_edge_coverage(self):
        ...
        tiny_prices = pd.Series([100.0, 100.01, 100.02], 
                              index=pd.date_range('2020-01-01', periods=3, freq='D'))
    ...
    def test_improved_ma_cross_full_parameter_coverage(self, mock_broker):
        ...
        test_prices = pd.Series([50000, 51000, 52000], 
                              index=pd.date_range('2020-01-01', periods=3, freq='D'))
        
        mock_broker.backtest_single.return_value = pd.Series([10000, 11000, 12000], 
    ...
    def test_main_block_coverage_simple(self):
        ...
        with patch('pandas.read_csv', return_value=test_df), \
            ...
            with patch('src.strategies.improved_strategy.broker') as mock_broker2:
                ...
                original_ma_equity = mock_broker2.backtest_single(btc, fast_win=7, slow_win=20, atr_win=20, init_equity=init_equity)
            ...
            with patch('src.strategies.improved_strategy.metrics') as mock_metrics:
                ...
                with patch('src.strategies.improved_strategy.signals') as mock_signals:
                    ...
                    mock_signals.moving_average.side_effect = [mock_ma_fast, mock_ma_slow, mock_ma_fast, mock_ma_slow]
...
class TestMainBlockRealExecution:
    ...
    def test_execute_main_block_with_import_tricks(self):
        ...
        with patch('pandas.read_csv', return_value=test_df), \
             ...
             patch('matplotlib.pyplot.title'), \
             patch('matplotlib.pyplot.ylabel'), \
             patch.dict('sys.modules', {'src.strategies.improved_strategy': sys.modules['src.strategies.improved_strategy']}):
            ...
            try:
                ...
                with patch('src.strategies.improved_strategy.metrics') as mock_metrics, \
                    ...
                    mock_signals.moving_average.return_value = pd.Series([50000] * 500, index=test_df.index)
                    mock_signals.bullish_cross_indices.return_value = [10, 50, 100]
                    mock_signals.bearish_cross_indices.return_value = [30, 80, 120]
                    
                    mock_broker.backtest_single.return_value = pd.Series([100000] * 500, index=test_df.index)
                    mock_broker.compute_position_size.return_value = 1.0
                    ...
                    try:
                        ...
                        tf_equity = trend_following(btc, long_win=200, atr_win=20, init_equity=init_equity)
                        improved_ma_equity = improved_ma_cross(btc, fast_win=50, slow_win=200, atr_win=20, init_equity=init_equity)
                        original_ma_equity = mock_broker.backtest_single(btc, fast_win=7, slow_win=20, atr_win=20, init_equity=init_equity)
...
class TestSuperTargetedLineCoverage:
    ...
    def setup_method(self):
        ...
        self.rising_prices = pd.Series([
            50000, 50100, 50200, 50300, 50400,  # 0-4: 初始上涨
            51000, 51500, 52000, 52500, 53000,  # 5-9: 继续上涨，触发多次移动止损更新
            53200, 53400, 53600, 53800, 54000   # 10-14: 持续上涨
        ] + [54000] * 35, index=self.dates)  # 填充剩余数据
        
        # 场景2：触发止损 (stop不为None，price < stop)
        self.stop_trigger_prices = pd.Series([
            50000, 51000, 52000, 53000, 54000,  # 0-4: 上涨进入位置
            53800, 53600, 53400, 53200, 52800,  # 5-9: 小幅回调
            51000, 49000, 47000, 45000, 44000   # 10-14: 大幅下跌触发止损
        ] + [44000] * 35, index=self.dates)
    ...
    def test_trailing_stop_exact_line_98_102(self, mock_broker):
        ...
        trailing_values = [48500, 49000, 49500, 50000, 50500, 51000] * 10  # 重复10次避免StopIteration
    ...
    def test_stop_none_to_value_transition(self, mock_broker):
        ...
        mock_broker.compute_trailing_stop.side_effect = ([None, 49000, 49500, 50000] * 15)  # 重复避免StopIteration
    ...
    def test_complete_main_block_simulation(self):
        ...
        with patch('pandas.read_csv', return_value=test_df), \
            ...
            original_ma_equity = pd.Series(
                100000 * (1 + np.random.randn(1000) * 0.01).cumprod(), 
                index=dates
            )
...
class TestMainBlockDirectCoverage:
    ...
    def test_main_block_via_name_main_simulation(self):
        ...
        with patch('pandas.read_csv', return_value=test_df), \
             ...
             patch('src.strategies.improved_strategy.signals') as mock_signals, \
             patch('src.strategies.improved_strategy.broker') as mock_broker, \
             patch('builtins.print') as mock_print:  # Mock print to avoid output
            ...
            mock_broker.backtest_single.return_value = pd.Series([100000] * len(btc_prices), index=dates)
            mock_broker.compute_position_size.return_value = 1.0
            mock_broker.compute_stop_price.return_value = 48000
            mock_broker.compute_trailing_stop.return_value = 49000
