...
class TestDataTransformersSimplified:
    ...
    def test_data_splitter_methods(self):
        ...
        data = pd.DataFrame({"feature": range(100), "target": np.random.choice([0, 1], 100)})
    ...
    def test_missing_value_handler_methods(self):
        ...
        data = pd.DataFrame({"col1": [1, 2, np.nan, 4, 5], "col2": [10, np.nan, 30, 40, np.nan]})
...
class TestBrokersSimplified:
    ...
    def broker_config(self):
        """broker配置"""
        return {"api_key": "test_key", "api_secret": "test_secret", "telegram_token": "test_token"}
    ...
    def test_broker_execute_order(self, broker_config):
        """测试broker订单执行"""
        with (
            patch.object(Broker, "_log_trade_to_csv"),
    ...
    def test_broker_trade_queries(self, broker_config):
        ...
        with patch(
            "pandas.read_csv",
            return_value=pd.DataFrame({"symbol": ["BTCUSDT"], "side": ["BUY"], "quantity": [1.0]}),
        ):
            broker = Broker(**broker_config)
...
class TestIntegrationSimplified:
    ...
    def test_full_data_pipeline(self):
        ...
        # 创建测试数据
        data = pd.DataFrame(
            {"price": [100, 101, np.nan, 103, 104], "volume": [1000, np.nan, 1200, 1300, 1400]}
        )
    ...
    def test_broker_monitoring_integration(self):
        ...
        # 创建broker
        broker = Broker(api_key="test", api_secret="secret", telegram_token="token")
