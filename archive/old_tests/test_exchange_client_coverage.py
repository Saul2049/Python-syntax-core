...
class TestExchangeClientInitialization:
    ...
    def test_initialization_demo_mode(self):
        """测试演示模式初始化"""
        client = ExchangeClient(api_key="demo_key", api_secret="demo_secret", demo_mode=True)
    ...
    def test_initialization_live_mode(self):
        """测试实盘模式初始化"""
        client = ExchangeClient(api_key="live_key", api_secret="live_secret", demo_mode=False)
    ...
    def test_load_demo_data_success(self, mock_read_csv):
        """测试成功加载演示数据"""
        # 模拟CSV数据
        mock_df = pd.DataFrame({"timestamp": ["2023-01-01", "2023-01-02"], "close": [100.0, 101.0]})
...
class TestExchangeClientRequestMethods:
    ...
    def demo_client(self):
        """创建演示模式客户端"""
        return ExchangeClient(api_key="demo_key", api_secret="demo_secret", demo_mode=True)

    @pytest.fixture
    def live_client(self):
        """创建实盘模式客户端"""
        return ExchangeClient(api_key="live_key", api_secret="live_secret", demo_mode=False)
...
class TestExchangeClientTradingMethods:
    ...
    def test_get_account_balance_live(self, mock_request):
        """测试实盘模式获取账户余额"""
        live_client = ExchangeClient(api_key="live_key", api_secret="live_secret", demo_mode=False)
    ...
    def test_place_order_demo_sell(self, mock_ticker, demo_client):
        ...
        order = demo_client.place_order(
            symbol="BTC/USDT", side="sell", order_type="limit", quantity=0.1, price=51000.0
        )
    ...
    def test_get_historical_trades_live(self, mock_request):
        ...
        mock_request.assert_called_once_with(
            "GET", "/api/v1/trades/BTC/USDT", params={"limit": 100}
        )
...
class TestExchangeClientKlinesMethods:
    ...
    def test_filter_demo_dates_with_time_range(self, demo_client):
        ...
        filtered_dates = demo_client._filter_demo_dates(market_data, start_time, end_time, 100)
    ...
    def test_get_api_klines(self, mock_request):
        ...
        mock_request.return_value = [[1640995200000, "50000", "51000", "49000", "50500", "100"]]

        klines = live_client._get_api_klines("BTC/USDT", "1d", 1640995200000, 1641081600000, 100)
