...
class TestBroker:
    ...
    def test_broker_initialization(self, mock_broker_config):
        """测试broker初始化"""
        with patch('src.notify.Notifier'), \
    ...
    def test_execute_buy_order(self, mock_broker_config):
        """测试执行买入订单"""
        with patch('src.notify.Notifier') as mock_notifier_class, \
...
class TestExchangeClient:
    ...
    def test_get_ticker_method(self, client_config):
        ...
        # 测试get_ticker方法（实际存在）
        with patch.object(client, '_demo_market_data', {"BTC/USDT": {datetime.now(): 50000}}), \
             patch('random.random', return_value=0.1):  # 避免触发5%的网络错误模拟
...
class TestBrokersIntegration:
    ...
    def test_broker_exchange_integration(self):
        ...
        with patch('src.notify.Notifier') as mock_notifier, \
             patch('src.core.position_management.PositionManager') as mock_pm:
    ...
    def test_full_trading_workflow(self):
        ...
        with patch('src.notify.Notifier') as mock_notifier, \
             patch('src.core.position_management.PositionManager') as mock_pm, \
             patch.object(Broker, '_get_mock_price', return_value=50000.0), \
             patch('random.random', return_value=0.9):  # 避免触发随机错误
    ...
    def test_error_scenarios(self):
        ...
        with patch('src.notify.Notifier') as mock_notifier, \
            ...
            # 测试网络错误处理 - 直接调用模拟错误方法
            with patch.object(client, '_simulate_demo_mode_issues', side_effect=ConnectionError("网络错误")):
                try:
