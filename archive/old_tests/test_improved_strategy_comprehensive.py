...
class TestTrendFollowingStrategy:
    ...
    def test_trend_following_different_parameters(self, trend_price_data):
        """测试不同参数的趋势跟踪"""
        # 测试不同的风险系数
        equity_low_risk = trend_following(
            trend_price_data, long_win=50, risk_frac=0.01
        )
        equity_high_risk = trend_following(
            trend_price_data, long_win=50, risk_frac=0.03
        )
    ...
    def test_trend_following_broker_integration(self, mock_stop, mock_size, mock_trailing, trend_price_data):
        """测试与broker模块的集成"""
...
class TestStrategyIntegration:
    ...
    def test_all_strategies_comparison(self, integration_data):
        ...
        with patch('src.broker.backtest_single') as mock_backtest:
            mock_backtest.return_value = pd.Series(
                [init_equity] * len(integration_data), 
                index=integration_data.index
            )
    ...
    def test_strategy_edge_cases(self):
        """测试策略边缘情况"""
        # 测试极端价格数据
        extreme_data = pd.Series([1e-6, 1e6, 0.001, 1000000], 
...
class TestStrategyRobustness:
    ...
    def test_buy_and_hold_with_inf_values(self):
        """测试包含无穷大值的数据"""
        data_with_inf = pd.Series([100, np.inf, 110, 120], 
                                 index=pd.date_range('2023-01-01', periods=4, freq='D'))
    ...
    def test_strategies_with_minimal_data(self):
        """测试最少数据情况"""
        minimal_data = pd.Series([100.0, 101.0], 
    ...
    def test_strategies_numerical_stability(self):
        """测试数值稳定性"""
        # 测试很小的价格值
        small_prices = pd.Series([0.001, 0.002, 0.0015, 0.0025], 
                                index=pd.date_range('2023-01-01', periods=4, freq='D'))
