...
class TestTimeSeriesProcessor:
    ...
    def test_create_lagged_features(self, time_series_data):
        """测试滞后特征创建"""
        processor = TimeSeriesProcessor()
        lagged = processor.create_lagged_features(
            time_series_data, ['value'], [1, 2, 3]
        )
    ...
    def test_create_rolling_features(self, time_series_data):
        """测试滚动特征创建"""
        processor = TimeSeriesProcessor()
        rolling = processor.create_rolling_features(
            time_series_data, ['value'], [5, 10]
        )
...
class TestDataSplitter:
    ...
    def test_rolling_window_split(self, sample_data):
        """测试滚动窗口分割"""
        splitter = DataSplitter()
        splits = splitter.rolling_window_split(
            sample_data, window_size=50, step_size=5
        )
...
class TestTransformersIntegration:
    ...
    def test_full_pipeline(self):
        ...
        data_with_lags = ts_processor.create_lagged_features(
            data_filled, ['price'], [1, 2, 3]
        )
        assert 'price_lag_1' in data_with_lags.columns
        
        # 3. 归一化数据
        normalizer = DataNormalizer(method='minmax')
        data_normalized = normalizer.fit_transform(data_with_lags.select_dtypes(include=[np.number]))
