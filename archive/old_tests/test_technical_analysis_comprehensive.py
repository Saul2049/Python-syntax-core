...
class TestTechnicalIndicators(unittest.TestCase):
    ...
    def test_add_moving_averages_invalid_column(self):
        ...
        with self.assertRaises(ValueError) as context:
            TechnicalIndicators.add_moving_averages(
                self.ohlcv_data, price_column="invalid_column"
            )
    ...
    def test_bollinger_bands_calculation(self):
        ...
        if len(valid_data) > 0:
            self.assertTrue(all(
                valid_data['BB_lower'] <= valid_data['BB_middle']
            ))
            self.assertTrue(all(
                valid_data['BB_middle'] <= valid_data['BB_upper']
            ))
    ...
    def test_macd_calculation(self):
        ...
        macd = TechnicalIndicators.macd(self.prices, fast_period=12, slow_period=26, signal_period=9)
...
class TestVolatilityIndicators(unittest.TestCase):
    ...
    def test_historical_volatility_calculation(self):
        ...
        vol = VolatilityIndicators.historical_volatility(
            self.prices, window=20, trading_days=252
        )
