...
class TestFinal100PercentCoverage:
    ...
    def test_trailing_stop_logic_coverage_99_103(self):
        ...
        with patch('src.strategies.improved_strategy.signals') as mock_signals, \
            ...
            mock_broker.compute_trailing_stop.side_effect = lambda *args, **kwargs: args[1] * 0.95 if len(args) > 1 else 47000.0
    ...
    def test_command_line_with_file_coverage_273_274(self):
        ...
        test_df = pd.DataFrame({
            'btc': 50000 + np.random.randn(100) * 100,
            'eth': 40000 + np.random.randn(100) * 80
        }, index=dates)
