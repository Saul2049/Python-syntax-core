...
class ExchangeClient:
    ...
    def _request(
        ...
        self._simulate_demo_mode_issues()
        self._apply_rate_limiting()

        return self._execute_request_with_retry(method, url, params, data)

    def _simulate_demo_mode_issues(self):
        """演示模式模拟网络问题"""
    ...
    def _execute_request_with_retry(self, method: str, url: str, params: dict, data: dict) -> dict:
        """执行带重试的请求"""
        for attempt in range(self.retry_count):
            try:
                return self._make_single_request(method, url, params, data)
            except (ConnectionError, Timeout, socket.error) as e:
                if attempt < self.retry_count - 1:
                    self._handle_retry_delay(attempt, e)
                else:
    ...
    def _make_single_request(self, method: str, url: str, params: dict, data: dict) -> dict:
        """执行单次HTTP请求"""
    ...
    def _handle_retry_delay(self, attempt: int, error: Exception):
        """处理重试延迟"""
        sleep_time = self.retry_delay * (2**attempt)  # 指数退避
        logger.warning(
            f"请求失败 (尝试 {attempt+1}/{self.retry_count}): " f"{str(error)}. 等待 {sleep_time}秒后重试..."
        )
        time.sleep(sleep_time)
    ...
    def get_historical_klines(
        ...
        """
        if self.demo_mode:
            return self._get_demo_klines(symbol, start_time, end_time, limit)

        return self._get_api_klines(symbol, interval, start_time, end_time, limit)

    def _get_demo_klines(
        self, symbol: str, start_time: Optional[int], end_time: Optional[int], limit: int
    ) -> List[List]:
        """获取演示模式的K线数据"""
        ...
        dates = self._filter_demo_dates(market_data, start_time, end_time, limit)

        return self._generate_demo_klines(dates, market_data)

    def _filter_demo_dates(
        self, market_data: dict, start_time: Optional[int], end_time: Optional[int], limit: int
    ) -> list:
        """过滤演示数据的日期范围"""
    ...
    def _generate_demo_klines(self, dates: list, market_data: dict) -> List[List]:
        """生成演示模式的OHLCV数据"""
        result = []
        for date in dates:
